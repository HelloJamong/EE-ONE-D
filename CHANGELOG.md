...
### Added
- PR 및 `main` 브랜치 변경을 자동 검증하는 GitHub Actions CI 추가
  - TypeScript 빌드와 회귀 테스트, Prisma 스키마 검증, npm 보안 감사를 수행
- 기존 `prisma db push` 설치 환경을 정식 마이그레이션 이력으로 안전하게 전환하는 초기 baseline migration과 준비 스크립트 추가
- `/config clear` 명령어 추가
  - 역할 패널, 관리자 설정, 감사 로그, 공지사항, 웰컴 메시지 채널 설정을 개별 초기화 가능
- Instagram 임베드, DB 배포, 권한·서버 격리, Discord 메시지 제한, 캐시 및 이벤트 오류 처리를 검증하는 회귀 테스트 추가

### Changed
- Instagram 미리보기 조회를 변경에 취약한 내부 GraphQL API에서 공개 oEmbed 응답 기반으로 변경
  - `/p/`, `/reel/`, `/reels/` 링크를 정규화하고 로그인·레이트리밋 응답과 실제 접근 불가 게시물을 구분
- Docker 시작 절차를 `prisma db push --accept-data-loss`에서 정식 `prisma migrate deploy` 방식으로 변경
  - `npm install` 대신 lockfile 기반 `npm ci` 사용
- Discord 비동기 이벤트 핸들러에 공통 오류 경계를 적용해 처리되지 않은 Promise 거부가 프로세스에 전파되지 않도록 개선
- 봇 상태 메시지는 `DISCORD_GUILD_ID`로 지정한 운영 서버에서만 변경하도록 다중 서버 동작을 명확히 제한
- 감사 이미지 캐시는 감사 로그가 활성화된 서버에서만 동작하며, 24시간 TTL과 주기적 만료 정리를 적용
- 릴리스 워크플로우에 테스트·Prisma 검증·보안 감사를 추가하고 Docker GHA 레이어 캐시를 사용하도록 개선
- Discord.js 및 관련 의존성을 업데이트하고 최소 Node.js 버전을 `20.18.1`로 조정
- 실제 명령 옵션과 현재 기능에 맞게 README 및 명령어 문서 정비

### Fixed
- 정상 공개 Instagram 게시물을 비공개·삭제 게시물로 잘못 안내하던 문제 수정
- `/welcome edit`에서 이미 응답한 interaction에 Modal을 표시해 실패하던 문제 수정
- 다른 Discord 서버의 역할 패널 ID로 조회·수정·삭제할 수 있던 서버 소유권 검증 누락 수정
- 커스텀 명령어의 `EMBED:제목|||내용` 응답이 제목과 본문으로 묶이지 않고 별도 랜덤 응답으로 분리되던 문제 수정
- 2,000자를 초과하는 일반 커스텀 응답을 Discord 허용 길이에 맞춰 분할 전송하도록 수정
- 커스텀 명령어와 공지사항 Modal 제출 시 관리자 권한 및 관리자 채널을 다시 검증하도록 수정
- DM 등 Discord 서버 밖에서 길드 전용 명령어를 실행할 때 발생할 수 있던 null 길드 오류 수정
- 공지사항 입력이 Discord 메시지 2,000자 제한을 초과하던 문제와 도움말 Embed 필드가 1,024자 제한을 초과하던 문제 수정
- 운영 의존성의 높은 등급 보안 취약점 제거

### Removed
- 더 이상 사용하지 않는 Instagram 내부 GraphQL 설정 환경변수 제거
- 이미 제거된 공지사항 투표 기능에 대한 오래된 명령어 문서 제거

## [26.6.4] - 2026-08-18

### Fixed
- 인스타그램 API가 레이트리밋/로그인 요구로 요청을 차단할 때 "비공개 계정, 삭제된 게시물" 메시지로 잘못 안내되던 문제 수정
  - 인스타그램 GraphQL이 차단 시 반환하는 `require_login`/`status: "fail"` 응답(`data`/`errors` 필드 없음)을 기존 코드가 걸러내지 못하고 media 없음(비공개·삭제) 케이스로 오분류
  - 해당 응답을 일시적 오류로 분류하도록 수정

### Changed
- 일시적 오류(레이트리밋 등) 발생 시 3초, 6초 간격으로 최대 2회 자동 재시도하도록 변경
  - 재시도 후에도 실패하면 원본 링크 메시지를 삭제하지 않고 보존해 사용자가 다시 보낼 필요 없도록 개선 (실제 비공개·삭제 게시물은 기존과 동일하게 삭제 + 안내)

## [26.6.3] - 2026-08-18

### Fixed
- 인스타그램 공유 링크의 손상된 쿼리 파라미터(`&igsi=...`) 인식 실패 수정
  - 인스타그램 앱 공유 시 첫 파라미터가 `?`가 아닌 `&`로 시작하는 링크(`.../reel/{shortcode}/&igsi=...`)가 `IG_REGEX`에 매치되지 않아 임베드가 전혀 발송되지 않던 문제
  - `IG_REGEX`의 쿼리 구분자를 `?` → `[?&]`로 확장하고, `postUrl` 파싱도 동일하게 `?`/`&` 둘 다 clean URL 경계로 처리하도록 수정

## [26.6.2] - 2026-07-06

### Added
- 인스타그램 미리보기를 가져올 수 없을 때 사유 안내 임베드 추가
  - 기존 `⚠️` 리액션만 남기던 방식 대신, "일시적 오류" / "비공개·삭제·로그인 필요 게시물"로 사유를 구분해 안내
  - 게시물을 직접 확인할 수 있도록 원본 링크를 클릭 가능한 형태로 포함

### Changed
- 인스타그램 임베드의 `setAuthor`를 게시물 작성자 → Discord 발신자(닉네임+아바타)로 변경
  - 원본 메시지를 삭제한 뒤에도 누가 링크를 공유했는지 알 수 있도록 디시인사이드 임베드·이모지 확대 기능과 동일한 방식으로 통일
  - 인스타그램 게시물 작성자 정보는 footer(`Instagram · 작성자명`)로 이동
  - 미리보기 실패 시 안내 임베드에도 동일하게 발신자 정보 표시

## [26.6.1] - 2026-07-06

### Fixed
- 인스타그램 임베드 하이퍼링크가 게시물이 아닌 인스타그램 메인 페이지(또는 작성자 프로필)로 이동하는 문제 수정
  - `buildEmbed()`에 `setTitle()`이 없어 `setURL()`이 무시되던 것이 원인 (Discord 임베드는 title 없이 url만 설정하면 링크가 렌더링되지 않음)
  - 게시물/동영상 여부에 따라 타이틀을 구분해 표시하고, 해당 타이틀이 게시물 URL로 연결되도록 수정
- 인스타그램 GraphQL `doc_id`/`X-IG-App-ID` 값 갱신
  - 기존 값이 Instagram 측 스키마 변경으로 모든 게시물에서 `execution error`를 반환하는 상태였음을 재현으로 확인
  - 검증된 신규 `doc_id`(`10015901848480474`) + 요청 헤더(`X-FB-LSD`, `X-ASBD-ID`)로 교체

### Changed
- 임베드 생성 실패 시 원본 메시지에 `⚠️` 리액션을 남기도록 변경 (링크 미감지와 감지 후 fetch 실패를 구분 가능)
- 인스타그램 GraphQL 에러 응답과 media 없음(비공개·삭제·제한 게시물) 케이스를 구분해 로깅
- `IG_DOC_ID`, `IG_APP_ID`를 환경변수로 분리 (`.env.example`에 문서화, 기본값 유지로 기존 배포에 영향 없음)
  - Instagram이 doc_id를 다시 회전시켜도 재배포 없이 `.env` 갱신만으로 대응 가능

## [26.06.00] - 2026-06-17

### Added
- 삭제 메시지 이미지 사전 캐싱: 이미지 첨부가 포함된 메시지를 게시 시점에 메모리 캐시(바이트 상한 LRU, 총 200MB·개별 10MB)에 보관 후, 삭제 시 캐시에서 원본을 복원해 감사 로그에 첨부 (`BoundedBufferCache`)
  - 디스코드 CDN URL 만료(~24h)나 캐시에서 밀려난 오래된(partial) 메시지로 인해 드물게 삭제 이미지가 기록되지 않던 문제를 해소

### Changed
- 메시지 삭제 감사 시 이미지 복원 경로를 캐시 우선으로 변경 (캐시 미스일 때만 삭제 시점 라이브 다운로드 폴백)
- 멤버 서버 입·퇴장(`guildMemberAdd`/`guildMemberRemove`) 핸들러에 진단용 `info` 로그 추가
- 감사 로그 전송 생략(비활성·로그 채널 미설정·채널 캐시 미스) 시 사유를 `debug` 로그로 기록

### Fixed
- `guildMemberRemove`에서 partial 멤버의 `user`가 없을 때 예외로 퇴장 이력이 누락될 수 있던 문제 방어
...
### Added
- 인스타그램 임베드 기능 (`igEmbed`)
  - 게시물(`/p/`), 릴스(`/reel/`, `/reels/`), IGTV(`/tv/`) URL 모두 지원
  - 단독 Instagram 링크 메시지를 Discord 임베드로 자동 교체
  - 게시물: 첫 번째 이미지 + 작성자 프로필 + 캡션
  - 릴스: 영상 썸네일(첫 프레임) + 작성자 프로필 + 캡션
  - `?utm_source`, `?igsh` 등 추적 파라미터 자동 제거
  - Instagram 내부 GraphQL API 활용 (추가 npm 의존성 없음)
  - CSRF 토큰 30분 캐싱, 미리보기 결과 5분 TTL 캐싱 적용
  - 비공개 계정·삭제 게시물은 조용히 실패 (원본 메시지 유지)
